...
Every generated markdown artifact with a title includes this line immediately
after the title:
...
Use the exact identifier exposed by the runtime; never guess or infer it. If the
runtime does not expose the identifier, write `model identifier unavailable`.
When updating an existing artifact with a different model, preserve the
original line and add `*Updated by: <exact runtime model identifier>*` below it.
